...
export default function EODFormTab({ leads, currentUserName, showSuccessModal, toast }) {
  ...
  useEffect(() => {
    function handleBeforeUnload(e) { if (submitting) { e.preventDefault(); e.returnValue = ''; } }
  ...
  async function handleSubmit() {
    ...
    if (missing.length > 0) { setResult({ ok: false, message: `Missing: ${missing.join(', ')}` }); return; }
    setSubmitting(true);
    setResult(null);
    try {
      const [hubspotEncoded, attendanceEncoded] = await Promise.all([fileToBase64(hubspotFile), fileToBase64(attendanceFile)]);
      postToSheet(eodPayload({ ...form, hubspotFile: hubspotEncoded, attendanceFile: attendanceEncoded })).then((res) => {
  ...
  return (
    <div className="card" style={{ padding: 20 }}>
      ...
      <div className="field">
        ...
        <textarea rows={3} placeholder="Client Name + Purpose, e.g. check-in or IAR" value={form.clientCalls} onChange={(e) => update('clientCalls', e.target.value)}
      ...
      <div className="field"><label>Hubspot tasks completion — screenshot of remaining tasks (max 10 MB)</label><input type="file" accept="image/*" onChange={(e) => setHubspotFile(e.target.files[0] || null)} /></div>
      <div className="field"><label>TP App attendance notification — screenshot (max 10 MB)</label><input type="file" accept="image/*" onChange={(e) => setAttendanceFile(e.target.files[0] || null)} /></div>
      <button className="primary" onClick={handleSubmit} disabled={submitting}>{submitting ? 'Submitting… keep this tab open' : 'Submit EOD'}</button>
      {result && (
